...
#
echo "DEBUG: Invoking IO stress tool for $DURATION seconds on $CPUS CPUs" > /dev/stderr
...
#
$STATSTOOL -S -T -X 100 -a $SAMPLES_LOG | grep metric: | sed 's/metric:/metric:stress_IO_Sync_/'

echo "DEBUG: statstool output:" > /dev/stderr
echo "DEBUG: -------------------------" > /dev/stderr
$STATSTOOL -S -T -X 100 -a $SAMPLES_LOG | grep metric: | sed 's/metric:/metric:stress_IO_Sync_/' > /dev/stderr
echo "DEBUG: -------------------------" > /dev/stderr
